...
const BOS_FRAMEWORK_ADDRESS: AccountAddress = address_from_single_byte(2);
...
/// Return true if `f` is a Bos framework function declared in `module` with a name in `names`
fn is_framework_function(f: &FunctionRef, module: &str, names: Vec<&str>) -> bool {
    *f.module_id.address() == BOS_FRAMEWORK_ADDRESS
