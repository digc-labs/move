...
pub enum PublicForTesting {
    ...
    Entry(Loc),
    // TODO we should allow calling init in unit tests, but this would need Bos bytecode verifier
...
pub fn public_testing_visibility(
    ...
    // is_testing && (is_entry || is_bos_init)
    if !env.flags().is_testing() {
        return None;
    }

    // TODO support bos init functions
    // let flavor = env.package_config(package).flavor;
    // flavor == Flavor::Bos && callee_name.value() == INIT_FUNCTION_NAME
