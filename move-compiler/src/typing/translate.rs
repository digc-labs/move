...
fn unused_module_members(
    ...
    let is_bos_mode = env.package_config(mdef.package_name).flavor == Flavor::Bos;
    ...
    for (loc, name, fun) in &mdef.functions {
        ...
        if is_bos_mode && *name == bos_mode::INIT_FUNCTION_NAME {
            // a Bos-specific filter to avoid signaling that the init function is unused
            continue;
        }
