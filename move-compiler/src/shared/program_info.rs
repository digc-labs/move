...
pub struct ProgramInfo<const AFTER_TYPING: bool> {
    ...
    pub bos_flavor_info: Option<BosInfo>,
}
...
impl TypingProgramInfo {
    pub fn new(
        ...
        if env
            .package_configs()
            .any(|(_, config)| config.flavor == Flavor::Bos)
        {
            let bos_flavor_info = BosInfo::new(pre_compiled_lib, modules, &info);
            info.bos_flavor_info = Some(bos_flavor_info);
        };
        info
    }
}
