...
use crate::{
    ...
    expansion::{
        ...
        name_validation::{
            ...
            IMPLICIT_STD_MEMBERS, IMPLICIT_STD_MODULES, IMPLICIT_BOS_MEMBERS, IMPLICIT_BOS_MODULES,
        },
        path_expander::{
...
fn default_aliases(context: &mut Context) -> AliasMapBuilder {
    ...
    let bos_address = maybe_make_well_known_address(context, loc, symbol!("bos"));
    ...
    // if bos is defined and the current package is in Bos mode, add implicit bos aliases
    if bos_address.is_some() && context.env().package_config(current_package).flavor == Flavor::Bos
    {
        let bos_address = bos_address.unwrap();
        modules.extend(
            IMPLICIT_BOS_MODULES
                .iter()
                .copied()
                .map(|m| (bos_address, m)),
        );
        members.extend(
            IMPLICIT_BOS_MEMBERS
                .iter()
                .copied()
                .map(|(m, mem, k)| (bos_address, m, mem, k)),
        );
    }
