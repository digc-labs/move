...
use crate::{
    ...
    bos_mode::{
        ...
        OBJECT_NEW_UID_FROM_HASH, RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_CREATE, BOS_ADDR_NAME,
        BOS_ADDR_VALUE, BOS_CLOCK_CREATE, BOS_SYSTEM_ADDR_VALUE, BOS_SYSTEM_CREATE,
        BOS_SYSTEM_MODULE_NAME, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT, UID_TYPE_NAME,
    },
};
...
pub const FRESH_ID_FUNCTIONS: &[(AccountAddress, Symbol, Symbol)] = &[
    (BOS_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW),
    (BOS_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (BOS_ADDR_VALUE, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(AccountAddress, Symbol, Symbol)] = &[
    (
        BOS_SYSTEM_ADDR_VALUE,
        BOS_SYSTEM_MODULE_NAME,
        BOS_SYSTEM_CREATE,
    ),
    (BOS_ADDR_VALUE, CLOCK_MODULE_NAME, BOS_CLOCK_CREATE),
    (
        BOS_ADDR_VALUE,
    ...
    (BOS_ADDR_VALUE, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
...
impl SimpleAbsInt for IDLeakVerifierAI<'_> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from {bos}::{object}::{new}. \
                Or for tests, it can come from {bos}::{ts}::{ts_new}",
                bos = BOS_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(&BOS_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
