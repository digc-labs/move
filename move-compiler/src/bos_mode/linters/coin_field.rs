...
//! This analysis flags uses of the bos::coin::Coin struct in fields of other structs. In most cases
//! it's preferable to use bos::balance::Balance instead to save space.
...
const COIN_FIELD_DIAG: DiagnosticInfo = custom(
    LINT_WARNING_PREFIX,
    Severity::Warning,
    LinterDiagnosticCategory::Bos as u8,
    LinterDiagnosticCode::CoinField as u8,
    "sub-optimal 'bos::coin::Coin' field type",
);
        ...
        if let N::StructFields::Defined(_, sfields) = &sdef.fields {
            for (_floc, _fname, (_, (_, ftype))) in sfields {
                if is_field_coin_type(ftype) {
                    let msg = "Sub-optimal 'bos::coin::Coin' field type. Using \
                        'bos::balance::Balance' instead will be more space efficient";
...
fn is_field_coin_type(sp!(_, t): &N::Type) -> bool {
    ...
    match t {
        ...
        T::Apply(_, tname, _) => {
            ...
            tname.is(&BOS_ADDR_VALUE, COIN_MOD_NAME, COIN_STRUCT_NAME)
        }
