...
//! ProgramInfo extension for Bos Flavor
//! Contains information that may be expensive to compute and is needed only for Bos
...
use crate::{
    ...
    bos_mode::{
        OBJECT_MODULE_NAME, BOS_ADDR_VALUE, TRANSFER_FUNCTION_NAME, TRANSFER_MODULE_NAME,
        UID_TYPE_NAME,
    },
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `bos::transfer::transfer`
    PrivateTransfer(Loc),
}
...
impl BosInfo {
    pub fn new(
        ...
        assert!(info.bos_flavor_info.is_none());
    ...
    fn visit_ty(
        ...
        match ty_ {
            ...
            N::Type_::Apply(_, sp!(_, tn_), _)
                if tn_.is(&BOS_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
